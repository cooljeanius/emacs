/* hexl.c: Convert files for Emacs Hexl mode.
...
#include <stdio.h>
#include <ctype.h>
#if defined(DOS_NT) || defined(HAVE_FCNTL_H)
# include <fcntl.h>
# if (defined(__DJGPP__) && (__DJGPP__ >= 2)) || defined(HAVE_IO_H)
#  include <io.h>
# endif /* djgpp 2+ || HAVE_IO_H */
#endif /* DOS_NT || HAVE_FCNTL_H */
#if defined(WINDOWSNT) || defined(HAVE_IO_H)
# include <io.h>
#endif /* WINDOWSNT || HAVE_IO_H */
...
#ifdef DOS_NT
# if (defined(__DJGPP__ && (__DJGPP__ >= 2)) || (defined(WINDOWSNT))
        if (!isatty(fileno(stdout)))
          setmode(fileno(stdout), O_BINARY);
# else
        (stdout)->_flag &= ~_IOTEXT; /* print binary */
        _setmode(fileno(stdout), O_BINARY);
# endif /* djgpp 2+ || WINDOWSNT */
#endif /* DOS_NT */
        for (;;)
          {
