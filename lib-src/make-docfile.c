...
static void start_globals(void)
{
  ...
  fprintf (outfile, "#ifndef EMACS_GLOBALS_H\n");
  fprintf (outfile, "# define EMACS_GLOBALS_H 1\n");
...
static void put_char(int ch, struct rcsoc_state *state)
{
  int out_ch;
  do
    {
...
static void scan_keyword_or_put_char(int ch, struct rcsoc_state *state)
{
  if (state->keyword
...
static void add_global(enum global_type type, char *name, int value)
{
...
static void close_emacs_globals(void)
{
  fprintf (outfile, "};\n");
  fprintf (outfile, "extern struct emacs_globals globals;\n");
  fprintf (outfile, "\n");
  fprintf (outfile, "#endif /* !EMACS_GLOBALS_H */\n");
  fprintf (outfile, "\n");
}
...
static int search_lisp_doc_at_eol(FILE *infile)
{
  int c = 0, c1 = 0, c2 = 0;
