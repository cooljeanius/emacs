...
# include "macfont.h"
#endif /* pre-10.5 || !USE_CT_GLYPH_INFO */

#import "macappkit.h"
#if (MAC_OS_X_VERSION_MAX_ALLOWED >= 1050) || defined(HAVE_OBJC_RUNTIME_H)
...
{
  union u { NSRect ns; CGRect cg; };

  return ((union u *)&cgrect)->ns;
}
...
NSRectToCGRect(NSRect nsrect)
{
  ...
  return ((union u *)&nsrect)->cg;
}

static inline NSPoint
NSPointFromCGPoint(CGPoint cgpoint)
{
  union u { NSPoint ns; CGPoint cg; };

  return ((union u *)&cgpoint)->ns;
}
...
NSPointToCGPoint (NSPoint nspoint)
{
  ...
  return ((union u *) &nspoint)->cg;
}

static inline NSSize
NSSizeFromCGSize(CGSize cgsize)
{
  union u { NSSize ns; CGSize cg; };

  return ((union u *)&cgsize)->ns;
}
...
+ (NSColor *)colorWithXColorPixel:(unsigned long)pixel
{
  ...
  components[0] = ((CGFloat)RED_FROM_ULONG(pixel) / (CGFloat)255.0f);
  components[1] = ((CGFloat)GREEN_FROM_ULONG(pixel) / (CGFloat)255.0f);
  components[2] = ((CGFloat)BLUE_FROM_ULONG(pixel) / (CGFloat)255.0f);
  components[3] = 1.0f;
...
#if (MAC_OS_X_VERSION_MIN_REQUIRED >= 1060) && defined(HAVE_CBLOCKS_LANGUAGE_FEATURE)
...
# else
	   argument:block
# endif /* USE_ARC && __clang__ && (__clang_major__ < 5) */
...
#endif /* 10.6+ && HAVE_CBLOCKS_LANGUAGE_FEATURE */
...
#endif /* __clang_major__ && (__clang_major__ >= 3) */

#if MAC_USE_AUTORELEASE_LOOP && defined(HAVE_CBLOCKS_LANGUAGE_FEATURE)
void
...
#endif /* MAC_USE_AUTORELEASE_LOOP && HAVE_CBLOCKS_LANGUAGE_FEATURE */
...
#if (MAC_OS_X_VERSION_MAX_ALLOWED >= 1060) && defined(HAVE_CBLOCKS_LANGUAGE_FEATURE)
static int
...
#ifndef MAC_APPLE_EVENT
# define MAC_APPLE_EVENT NO_EVENT
...
{
  EmacsFrameController *frameController = FRAME_CONTROLLER(f);
  NSRect *rects = (NSRect *)alloca(sizeof(NSRect) * n);
  int i;
  XRectangle *xrects = (XRectangle *)alloca(sizeof(XRectangle) * n);

  for (i = 0; i < n; i++) {
    CONVERT_TO_XRECT(xrects[i], rectangles[i]);
    rects[i] = NSRectFromCGRect(mac_rect_make(f, xrects[i].x, xrects[i].y,
                                              xrects[i].width,
                                              xrects[i].height));
  }
...
#if (MAC_OS_X_VERSION_MIN_REQUIRED >= 1090) && defined(HAVE_CBLOCKS_LANGUAGE_FEATURE)
static NSArray *
...
#endif /* 10.9+ && HAVE_CBLOCKS_LANGUAGE_FEATURE */
  ...
  while (i-- > 0)
      ...
      if (displayInfo)
	{
      ...
      attributes = Fcons(Fcons(Qmm_size,
                               list2i((size.width + (CGFloat)0.5f),
                                      (size.height + (CGFloat)0.5f))),
			 attributes);
...
#define PROP(IDX) AREF(f->tool_bar_items, i * TOOL_BAR_ITEM_NSLOTS + (IDX))
      ...
      int idx = 0;
      ptrdiff_t img_id;
      struct image *img = (struct image *)alloca(sizeof(struct image));
      Lisp_Object image = LISP_INITIALLY_ZERO;
      NSString *label = @"";
      NSString *identifier = TOOLBAR_ICON_ITEM_IDENTIFIER;
      ...
      else
	{
	  ...
	  if (use_multiimage_icons_p)
	    [frameController updateBackingScaleFactor];
          ...
          if (img == NULL)
            continue; /* FIXME: is this the correct thing to do? */
  ...
  if (err == noErr)
    {
      err = mac_store_event_ref_as_apple_event(0, 0, Qfont, Qpanel_closed,
                                               event, 0, NULL, NULL);
      if (err != noErr) {
        ; /* FIXME: do something, like report the error... */
      }
      ReleaseEvent(event);
    }
}
...
- (int)noteMouseMovement:(NSPoint)point
{
  ...
  int x, y;
  NativeRectangle *r;
  XRectangle *xr = (XRectangle *)alloca(sizeof(XRectangle));
...
# endif /* 10.5+ */
  ...
  CONVERT_TO_XRECT_PTR(xr, r);
  /* Has the mouse moved off the glyph it was on at the last sighting?  */
  if ((f != dpyinfo->last_mouse_glyph_frame)
      || ((x < xr->x) || ((x - xr->x) >= xr->width) || (y < xr->y)
          || ((y - xr->y) >= xr->height)))
    {
      f->mouse_moved = 1;
      [emacsView lockFocus];
      ...
      unset_global_focus_view_frame();
      [emacsView unlockFocus];
      /* Remember which glyph we are now on: */
...
mac_font_dialog (struct frame *f)
{
  ...
  selectedFont = (__bridge NSFont *)macfont_get_nsctfont((struct font *)FRAME_FONT(f));
  ...
  if ([mainScreen canShowMenuBar])
    {
      ...
      point.v += (0 - NSMaxY(mainScreenFrame) + NSMaxY(baseScreenFrame));
    }
...
- (instancetype)initWithWidgetValue:(widget_value *)wv
{
  ...
  buttons = ((NSButton * __unsafe_unretained *)
	     alloca((sizeof(NSButton *) * nb_buttons) + 1UL));
     ...
     bottom-right corner of the inner box.  */
  {
	...
	button = buttons[i];
	frameRect = [button frame];
	button_cell_width = (NSWidth(frameRect)
                             - (DIALOG_BUTTON_BORDER * 2));
	button_cell_height = (NSHeight(frameRect)
                              - (DIALOG_BUTTON_BORDER * 2));
	if ((right - button_cell_width) < (0 - inner_width))
	  {
	    if ((i != first_group_count)
		&& ((right - button_cell_width)
                    >= (0 - DIALOG_MAX_INNER_WIDTH)))
	      inner_width = (0 - (right - button_cell_width));
	    else
	      {
    ...
    left_align_shift = (0 - (inner_width + NSMinX(frameRect)
                             + DIALOG_BUTTON_BORDER));
    for (i = (nb_buttons - 1); (i >= first_group_count) && (i >= 0); i--)
      {
	button = buttons[i];
	frameRect = [button frame];
...
create_and_show_dialog(struct frame *f, widget_value *first_wv)
{
  ...
  popup_activated_flag = 1;
  {
    ...
    do {
      ...
      /* This is necessary on 10.5 to make the dialog visible when the
       * user tries logout/shutdown: */
      [panel makeKeyAndOrderFront:nil];
...
#if defined(_NATIVE_OBJC_EXCEPTIONS) || defined(NS_IMPL_COCOA)
	    @try
	      {
...
#else
            if (keyPath != (NSString *)NULL) {
              ; /* FIXME: do something here? */
            }
            value = nil; /* FIXME: initialize more intelligently */
#endif /* _NATIVE_OBJC_EXCEPTIONS || NS_IMPL_COCOA */
               ...
               ptrdiff_t nargs, Lisp_Object *args, Lisp_Object *error_data)
{
  ...
  NSAttributedString *displayValue = (NSAttributedString *)NULL;

  *error_data = Qnil;
  script = mac_osa_create_script(code_or_file, compiled_p_or_language,
                                 file_p, error_data);
  if (script)
    {
      NSDictionary *errorInfo;
	...
	*error_data = mac_osa_error_info_to_lisp(errorInfo);
	...
	{
	  event =
            mac_apple_event_descriptor_with_handler_call(handler_call,
                                                         nargs, args);
	  if (event == nil)
	    *error_data = Fcons(build_string("Cannot create Apple event"),
...
{
#if 0
  /* the type of XImagePtr is incomplete in this fork: */
  XImagePtr ximg;
#else
  /* what Y.M.'s port typedefs XImagePtr as: */
  MacPixmap ximg;
#endif /* 0 */
  CGContextRef context;
  NSGraphicsContext *gcontext;
  ...
  ximg = (MacPixmap)mac_create_pixmap((NSWidth(rect) * scaleFactor),
...
/* FIXME: figure out how to do this the correct way: */
#if 1
- (NSUInteger)pageCount
{
  return (NSUInteger)0U;
}
#endif /* 1 */
...
- (CGColorRef)copyBackgroundCGColorOfPageAtIndex:(NSUInteger)index;
{
  ...
  /* `backgroundColor' might be nil, but that is OK: */
...
@end // EmacsDocumentRasterizer implementation
...
document_cache_evict(void)
{
  ...
  if ([documentRasterizerCacheOldestTimestamp timeIntervalSinceNow]
      > (0 - DOCUMENT_RASTERIZER_CACHE_DURATION))
    return;
...
void
mac_document_draw_page(CGContextRef c, CGRect rect,
                       EmacsDocumentRef document, size_t index)
{
  id <EmacsDocumentRasterizer> documentRasterizer =
    (__bridge id <EmacsDocumentRasterizer>) document;
...
mac_start_animation(Lisp_Object frame_or_window, Lisp_Object properties)
{
  ...
  CALayer *layer, *contentLayer;
  Lisp_Object direction = LISP_INITIALLY_ZERO;
  Lisp_Object duration;
  CGFloat h_ratio = 0.0f, v_ratio = 0.0f;
  enum {
    ANIM_TYPE_NONE,
    ANIM_TYPE_MOVE_OUT,
    ANIM_TYPE_MOVE_IN,
		 ...
		 BOOL screen_font_p)
{
  ...
  NSPoint spaceLocation;
  NSUInteger used, numberOfGlyphs = 0U;
...
#define RIGHT_TO_LEFT_P permutation
	 ...
	 setup a permutation for right-to-left text.  */
      ...
      for (range = NSMakeRange(0, 0); NSMaxRange(range) < used;
	   range.length++)
	{
	  struct mac_glyph_layout *gl = (glyph_layouts
                                         + NSMaxRange(range));
	  ...
	  if (characterIndex >= NSMaxRange(compRange))
		...
		for (i = 0; i < range.length; i++) {
		  permutation[range.location + i] = (NSMaxRange(range)
                                                     - i - 1);
                }
              }
      ...
      for (i = 0; i < used; i++)
	{
	  ...
	  for (nextGlyphIndex = (glyphIndex + 1);
               nextGlyphIndex < numberOfGlyphs;
	       nextGlyphIndex++)
            {
              if (![layoutManager
...
#endif /* CA_BASIC_ANIMATION */
#ifdef CA_MEDIA_TIMING_FUNCTION
# undef CA_MEDIA_TIMING_FUNCTION
#endif /* CA_MEDIA_TIMING_FUNCTION */
#ifdef NS_ANIMATION_CONTEXT
# undef NS_ANIMATION_CONTEXT
#endif /* NS_ANIMATION_CONTEXT */
#ifdef CA_CONSTRAINT
# undef CA_CONSTRAINT
#endif /* CA_CONSTRAINT */
#ifdef CA_CONSTRAINT_LAYOUT_MANAGER
# undef CA_CONSTRAINT_LAYOUT_MANAGER
#endif /* CA_CONSTRAINT_LAYOUT_MANAGER */
#ifdef BEGIN_AUTORELEASE_POOL_BLOCK_INPUT
# undef BEGIN_AUTORELEASE_POOL_BLOCK_INPUT
#endif /* BEGIN_AUTORELEASE_POOL_BLOCK_INPUT */
#ifdef END_AUTORELEASE_POOL_BLOCK_INPUT
# undef END_AUTORELEASE_POOL_BLOCK_INPUT
#endif /* END_AUTORELEASE_POOL_BLOCK_INPUT */
#ifdef Z
# undef Z
#endif /* Z */
